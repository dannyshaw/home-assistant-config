...
_FIND_QUERY = (
    "./forecast/area[@type='location']/forecast-period[@index='{}']/*[@type='{}']"
)
...
def setup_platform(hass, config, add_entities, discovery_info=None):
    ...
    if not product_id:
        product_id = closest_product_id(hass.config.latitude, hass.config.longitude)
        ...
        for index in range(start, config.get(CONF_DAYS) + 1):
            for condition in monitored_conditions:
                add_entities(
                    [
                        BOMForecastSensor(
                            bom_forecast_data, condition, index, name, product_id
                        )
                    ]
                )
...
class BOMForecastSensor(Entity):
    ...
    def name(self):
        ...
        if not self._name:
            return "BOM {} {}".format(SENSOR_TYPES[self._condition][1], self._index)
        return "BOM {} {} {}".format(
            self._name, SENSOR_TYPES[self._condition][1], self._index
        )
...
class BOMForecastSensorFriendly(Entity):
    ...
    def state(self):
        ...
        for condition in self._conditions:
            friendly_state = friendly_state.replace(
                "{{{}}}".format(condition),
                self._bom_forecast_data.get_reading(condition, self._index),
            )
        ...
        if condition == "detailed_summary":
            if PRODUCT_ID_LAT_LON_LOCATION[self._product_id][3] == "City":
                ...
                detailed_summary = self._data.find(
                    _FIND_QUERY.format(index, "forecast")
                ).text
            return (
                (detailed_summary[:251] + "...")
                if len(detailed_summary) > 251
